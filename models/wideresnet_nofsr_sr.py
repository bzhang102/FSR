...
class WideResNet(nn.Module):
    def __init__(self, depth=34, num_classes=10, widen_factor=10, dropRate=0.0, image_size=(32, 32), dx=0, dy=0):
    ...
    def forward(self, x, is_eval=False):
        ...
        out = self.block2(out)
        out = self.block3(out)
        feature_field = self.relu(self.bn1(out))
        ...
        # pool and classify
        out = F.avg_pool2d(feature_field_ensembled, 8)
...
def WideResNet34_NoFSR_SR(num_classes=10, image_size=(32, 32), dx=3, dy=3):
    return WideResNet(num_classes=num_classes, image_size=image_size, dx=dx, dy=dy)